...
def check_projection_lat_lon(chip_bounds, item, verbose=True):
    """Ensure that original chip and PySTAC item have the same coordinate projection"""
    
    if chip_bounds["crs"] == item.properties["proj:epsg"]:
        if verbose:
            logger.debug(
                f"""GeoTIFF and STAC item have same CRS {geotiff.meta["crs"]}"""
            )
...
def query_bands_from_lat_lon(
    ...
    """
    # Convert bounds to regular lat/long coordinates
    left, bottom, right, top = lat_lon['left'], lat_lon['bottom'], lat_lon['right'], lat_lon['top'] 
...
#    print("DEBUG", geotiff.meta["crs"], geotiff.bounds, left, bottom, right, top)
#    bbox = rasterio.coords.BoundingBox(left, bottom, right, top)
#    print(rasterio.warp.transform_bounds(4326, 32736, *bbox))
#    sys.exit()

    assets = {}
    for it, item in enumerate(items):
        bounds = check_projection_lat_lon(lat_lon, item, verbose)
